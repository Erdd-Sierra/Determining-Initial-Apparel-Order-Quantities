...
def solve_optimization_problem(demands, products, config, min_lot_size):
    ...
    目的関数: 機会損失（卸売価格の24%）と売れ残り損失（卸売価格の8%）の和を最小化
    ...
    # 最適化問題の定義
    prob = LpProblem("OrderOptimization", LpMinimize)
    ...
    # 目的関数: 総損失の最小化
    # 機会損失 = UnderageCost（Price × 24%）× shortage
    # 売れ残り損失 = OverageCost（Price × 8%）× surplus
    prob += lpSum([products[i]["UnderageCost"] * shortage[i] + 
                   products[i]["OverageCost"] * surplus[i] 
                   for i in range(n_products)]), "TotalLoss"
    ...
    if LpStatus[prob.status] == "Optimal":
        optimal_orders = [x[i].varValue if x[i].varValue is not None else 0 
                         for i in range(n_products)]
        total_loss = prob.objective.value()
        return optimal_orders, total_loss
    else:
        return None, None
...
def run_monte_carlo_simulation(products, config, min_lot_size, n_simulations):
    ...
    for sim in range(n_simulations):
        ...
        # 最適化問題を解く
        optimal_orders, total_loss = solve_optimization_problem(
    ...
    return recommended_orders, all_losses, all_results
...
def main():
    ...
    with tab1:
        ...
        st.info(f"機会損失: 卸売価格の24% | 売れ残り損失: 卸売価格の8% | 最小ロット数: {min_lot_size}")
        
        if st.button("シミュレーション実行", key="hk_button"):
            with st.spinner("シミュレーション実行中..."):
                recommended_orders, losses, results = run_monte_carlo_simulation(
                    PRODUCTS, CONFIG, min_lot_size, n_simulations
                )
                
                if recommended_orders is not None:
                    st.success(f"シミュレーション完了 ({len(losses)}回成功)")
                    ...
                    with col2:
                        ...
                        if losses:
                            ...
                            st.metric("平均損失", f"${avg_loss:,.2f}")
                            st.metric("最小損失", f"${min_loss:,.2f}")
                            st.metric("最大損失", f"${max_loss:,.2f}")
                            st.metric("標準偏差", f"${std_loss:,.2f}")
